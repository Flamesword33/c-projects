 ...
 *
 * This version uses file locking to ensure mutually exclusive access to the
 * critical section in which an account record is updated.
 * 
 *  J. Mohr
 *  2019-03-21
 */
...
int main(int argc, char *argv[])
{
	...
	while (fscanf(fp, "%u", &acct_num) != EOF)
	{
		...
		// Lock the current account record
		if (lockf(db_fd, F_LOCK, record_size) == -1)
			fatalsys("File locking failed");
		...
		// Seek again and unlock the current account record
		if (lseek(db_fd, acct_num * record_size, SEEK_SET) == -1)
			fatalsys("Seek failed");
		if (lockf(db_fd, F_ULOCK, record_size) == -1)
			fatalsys("File unlocking failed");
	}
